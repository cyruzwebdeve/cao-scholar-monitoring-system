...
const email = (process.argv[2] || 'moderator@example.com').toLowerCase().trim();
const password = process.argv[3] || 'Moderator@123';

async function main() {
  const passwordHash = await bcrypt.hash(password, 12);
  const existing = await prisma.admins.findFirst({ where: { email } });
  const data = { full_name: 'Content Moderator', email, password_hash: passwordHash, is_active: true, is_super_admin: false, role: 'moderator' };
  ...
  console.log(`Moderator account ready: ${email}`);
