...
const resolvePortalRole = (admin) => {
  ...
  if (admin.role === 'billing') return 'BillingPayrollAdmin';
  return null;
};
...
const serializeStaff = (admin, currentUserId = null) => {
  ...
  const roleConfig = STAFF_ROLE_CONFIG[role];
  return {
    id: admin.id,
    fullName: admin.full_name,
    email: admin.email,
    role: role || 'RetiredModerator',
    roleLabel: roleConfig?.label || 'Content Moderator (retired)',
    isActive: admin.is_active,
    sectionAccess: role ? normalizeSectionAccess(admin.section_access, role) : [],
    availableSections: role ? getAllowedSectionsForRole(role) : [],
