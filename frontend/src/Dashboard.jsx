...
function ApplicantsManagement({ token }) {
  ...
  return <>
    <div className="applicants-management">
      ...
      <div className="applicant-metrics">{metricCards.map(([label, value, Icon, tone]) => <article className={tone} key={label}><div><span>{label}</span><strong>{value}</strong><small>{label === 'Total Applicants' ? 'Registered records' : label === 'Scheduled for Exam' ? 'With examination slots' : label === 'Exam Completed' ? 'Results submitted' : 'Qualified applicants'}</small></div><i className={`applicant-metric-icon ${tone}`}><Icon size={20} strokeWidth={2.2} /></i></article>)}</div>
